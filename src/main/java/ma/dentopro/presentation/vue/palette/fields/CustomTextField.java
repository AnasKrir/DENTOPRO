package ma.dentopro.presentation.vue.palette.fields;
...
public class CustomTextField extends JTextField {
    ...
    private Color gainColor = new Color(100, 150, 255); // Couleur du texte au focus
    ...
    public CustomTextField(String hint) {
        ...
        setBorder(new EmptyBorder(5, 10, 5, 10)); // Espacement interieur
        lostFont = getFont(); // Utiliser la police par defaut comme `lostFont`
        gainFont = lostFont.deriveFont(Font.BOLD); // Police en gras pour le focus
        setForeground(lostColor); // Couleur par defaut du texte

        // Ajouter un FocusListener pour gerer le focus
        addFocusListener(new FocusListener() {
            ...
            public void focusLost(FocusEvent e) {
                if (getText().isEmpty()) {
                    setText(hint); // Reafficher le hint
                    setFont(lostFont);
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Dessiner le hint si necessaire
        if (getText().isEmpty() && !isFocusOwner()) {
            drawHint(g2);
        }

        // Appeler le parent pour gerer le texte saisi
    ...
    // Methodes pour configurer les proprietes dynamiquement
