...
public class PatientDao implements IDao<Patient,Long> {
    ...
    static File fileBase = new File("myFileBase/patients.txt");
    ...
    private static void writePatientToFile(Patient patient) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("myFileBase/patients.txt", true))) {
    ...
    public boolean deleteById(Long patientId) {
        // Lire tous les patients depuis le fichier
        List<Patient> patients = readPatientsFromFile(fileBase);
        Iterator<Patient> iterator = patients.iterator();
        boolean removed = false;

        // Parcourir la liste des patients pour trouver celui à supprimer
        while (iterator.hasNext()) {
            Patient patient = iterator.next();
            if (patient.getId().equals(patientId)) {
                iterator.remove(); // Supprimer le patient de la liste
                removed = true;
                break;
            }
        }
        ...
        if (removed) {
            // Mettre à jour le fichier des patients après suppression
            updatePatientsFile(patients);

            // Supprimer le dossier médical associé au patient
            DossierMedicaleDao dossierMedicaleDao = new DossierMedicaleDao();
            dossierMedicaleDao.deleteDossierMedicale(patientId); // Supprime les dossiers médicaux du patient
        }
    ...
    private void updatePatientsFile(List<Patient> patients) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileBase))) {
            for (Patient patient : patients) {
                writer.write(convertPatientToLine(patient)); // Convertir le patient en ligne de texte
                writer.newLine();
            }
        } catch (IOException e) {
            e.printStackTrace();
